...
try:
    ...
    try:
        ...
        # CPU-only env (e.g. CI): flash_attn wheels are CUDA-only and not
        # installed. Fall back to torch SDPA (same pattern as
        # skin_vae/attention_processor.py) so the model still runs on CPU.
