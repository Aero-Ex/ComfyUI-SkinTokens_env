...
def get_device_type():
    try:
        ...
        # CPU-only machine (e.g. CI): no CUDA device to query. Callers only
        # test for substrings like "H100", so "cpu" is a safe answer and --
        # critically -- must not raise at module import time (FLASH3 reads
        # this in __init__ during import; raising kills node registration).
        return "cpu"
