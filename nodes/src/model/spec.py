...
class ModelSpec(pl.LightningModule, ABC):
    ...
    def load_from_system_checkpoint(cls, checkpoint_path: str, strict: bool=True, **kwargs):
        ...
        if not os.path.isabs(checkpoint_path):
            ...
            try:
                import folder_paths
                full_path = folder_paths.get_full_path("skintoken", checkpoint_path)
                if full_path and os.path.exists(full_path):
                    found_path = full_path
                else:
                    for folder in folder_paths.get_folder_paths("skintoken"):
                        p = os.path.join(folder, checkpoint_path)
                        if os.path.exists(p):
                            found_path = p
                            break
            except Exception:
            ...
            if found_path:
                ...
                nodes_root = os.path.dirname(os.path.dirname(os.path.dirname(cur_file)))
                pack_root = os.path.dirname(nodes_root)
                comfy_root = os.path.dirname(os.path.dirname(pack_root))
                candidates = [
                    os.path.join(getattr(folder_paths, "models_dir", ""), "skintoken", checkpoint_path) if "folder_paths" in locals() else "",
                    os.path.join(comfy_root, "models", "skintoken", checkpoint_path),
                    os.path.join(pack_root, "models", "skintoken", checkpoint_path),
                    os.path.join(nodes_root, "models", "skintoken", checkpoint_path),
                ]
