        ...
        if skeleton_path is not None:
            ...
            for (cls, path) in skeleton_path.items():
                ...
                if not os.path.isabs(path):
                    nodes_dir = os.path.dirname(os.path.dirname(os.path.dirname(os.path.abspath(__file__))))
                    cand = os.path.join(nodes_dir, path)
                    if os.path.exists(cand):
                        path = cand
                    else:
                        pack_dir = os.path.dirname(nodes_dir)
                        cand2 = os.path.join(pack_dir, path)
